...
# Function to handle the reply when free users run the /bgmi1
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in /bgmi1_cooldown and (datetime.datetime.now() - /bgmi1_cooldown[user_id]).seconds < 3:
                response = "You Are On Cooldown . Please Wait 5min Before Running The /bgmi1 Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                ...
                full_command = f"./ydv {target} {port} {time} 70"
